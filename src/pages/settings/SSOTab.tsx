...
function SSOConfigForm({
  ...
  const mut = useMutation({
    mutationFn: () => {
      ...
      if (!form.x509_cert) {
        throw new Error('IdP certificate is required when creating.')
      }
    ...
    onError: (err: any) => {
      ...
      setError(d?.detail || d?.errors?.[0]?.message || err.message || 'Error')
    },
  })
    ...
    <form onSubmit={e => { e.preventDefault(); setError(''); mut.mutate() }} className="space-y-4 max-h-[70vh] overflow-y-auto pr-1">
      ...
      <div>
        <label className="label">
          IdP X.509 certificate {initial ? '(leave blank to keep existing)' : '*'}
        </label>
        <textarea
          required={!initial}
      ...
      <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
        <div>
          <label className="label">Default role (JIT) *</label>
        ...
        <div>
          <label className="label">Default teams *</label>
          <select
            multiple
          ...
          <p className="text-xs text-gray-400 mt-1">Ctrl+click for multi-select</p>
        </div>
      </div>
      ...
      <div className="flex flex-wrap gap-4 text-sm">
        {[
          ...
          ['enforce_sso', 'Enforce SSO (block password login)'],
          ['is_enabled', 'Enable this config'],
  ...
  return (
    <div>
      <div className="flex justify-between items-start mb-4 gap-4">
        <div>
          <p className="text-sm text-gray-500">{list.length} SSO configuration(s)</p>
          <p className="text-xs text-gray-400 mt-1">
            Guide: see <code className="bg-gray-100 px-1 rounded">docs/SSO_GUIDE.md</code> in the backend
          </p>
        </div>
        <button
      ...
      {!list.length ? (
        <EmptyState
          ...
          description="Connect Okta, Azure AD, or Google Workspace for federated authentication."
        />
      ) : (
            ...
            <div key={cfg.id} className="bg-white border border-gray-200 rounded-xl p-5">
              <div className="flex items-start justify-between gap-4">
                <div className="flex-1 min-w-0">
                  <div className="flex items-center gap-2 flex-wrap mb-1">
                    ...
                    {cfg.is_enabled ? (
                      ...
                      <span className="text-xs bg-gray-50 text-gray-500 px-2 py-0.5 rounded-full">Inactive</span>
                    )}
                    {cfg.enforce_sso && (
                      <span className="text-xs bg-amber-50 text-amber-700 px-2 py-0.5 rounded-full">SSO enforced</span>
                    )}
                  </div>
                  <p className="text-xs text-gray-400">Created on {formatDate(cfg.created_at)}</p>
                </div>
                <div className="flex gap-2 shrink-0">
                  ...
                  >
                    {expandedId === cfg.id ? 'Hide' : 'SP metadata'}
                  </button>
                  <button
                  ...
                  <button
                    onClick={() => { if (confirm('Delete this SSO config?')) deleteMut.mutate(cfg.id) }}
              ...
              {expandedId === cfg.id && cfg.sp_metadata && (
                <div className="mt-4 pt-4 border-t border-gray-100 space-y-2 text-xs">
                  <div className="flex items-start justify-between gap-2">
                    ...
                    <CopyButton text={cfg.sp_metadata.entity_id} label="Copy" />
                  </div>
                  <div className="flex items-start justify-between gap-2">
                    <div>
                    ...
                    <CopyButton text={cfg.sp_metadata.acs_url} label="Copy" />
                  </div>
                  <div className="flex items-start justify-between gap-2">
                    <div>
                    ...
                    <CopyButton text={`${window.location.origin}/scim/v2/`} label="Copy" />
                  </div>
      ...
      <Modal
        ...
        title={editConfig ? 'Edit SSO configuration' : 'New SSO configuration'}
      >
        <SSOConfigForm
      ...
      <Modal open={!!newScimToken} onClose={() => setNewScimToken(null)} title="SCIM token — copy it now">
        <p className="text-sm text-gray-600 mb-3">
          This token will not be shown in full again. Configure it in your IdP (Okta/Azure AD → SCIM).
        </p>
        <code className="block bg-gray-100 p-3 rounded-lg text-xs break-all font-mono">{newScimToken}</code>
        <div className="flex justify-end mt-4">
          <CopyButton text={newScimToken || ''} label="Copy token" />
        </div>
      </Modal>
    </div>
  )
