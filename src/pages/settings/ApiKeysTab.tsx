  ...
  return (
    <div>
      <div className="flex flex-col gap-3 sm:flex-row sm:justify-between sm:items-center mb-4">
        <p className="text-sm text-gray-500">{(keys as ApiKey[])?.length ?? 0} active key(s)</p>
      ...
      {!(keys as ApiKey[])?.length ? (
        <EmptyState icon={<IconLock size={24} />} title="No API keys" description="Generate keys to access the API from your scripts." />
      ) : (
            ...
            <div key={k.id} className="card-item flex flex-col sm:flex-row sm:items-start sm:justify-between gap-4">
              <div className="flex-1 min-w-0">
                ...
                <div className="text-xs text-gray-400 flex flex-col sm:flex-row sm:flex-wrap gap-1 sm:gap-4">
                  <span>Created on {formatDate(k.created_at)}</span>
                  {k.last_used_at && <span>Last used: {formatDate(k.last_used_at)}</span>}
                  {k.expires_at && <span>Expires: {formatDate(k.expires_at)}</span>}
                </div>
              </div>
              <Button variant="danger" size="sm" onClick={() => { if (confirm('Revoke this key?')) revokeMut.mutate(k.id) }}>
                Revoke
              </Button>
            </div>
          ))}
      ...
      <Modal open={showCreate && !newKey} onClose={() => setShowCreate(false)} title="New API key" size="lg">
      ...
      <Modal open={!!newKey} onClose={() => { setNewKey(null); setShowCreate(false) }} title="Copy your key now" size="md">
        <div className="space-y-4">
          <div className="bg-amber-50 border border-amber-200 rounded-lg p-4 text-sm text-amber-800">
            This key will only be shown <strong>once</strong>. Copy it and store it securely.
          </div>
